...
def parse_gpu_data(raw_data: list[dict], db: Session):
    ...
    for item in raw_data:
        ...
        slot_info = {
            "flavor": flavor,
            "compute": 0 if not is_mig else int(flavor[0]),  # Example: 2g.20gb → 2
            "user": user,
            "status": "RUNNING" if pod_name else "EMPTY"
        }

        # If MIG: slot per MIG unit, otherwise: entire GPU
        if is_mig:
            result[node][gpu_id].append(slot_info)
        else:
            result[node][gpu_id] = [slot_info]  # Not MIG: single slot
